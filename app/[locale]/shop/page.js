...
function ShopContent() {
  ...
  const filteredProducts = useMemo(() => {
    ...
    if (activeBrand !== "all")
      filtered = filtered.filter(
        (p) => (p.brandId || p.brand_id) === activeBrand,
      );
    ...
    if (brandsFilter) {
      ...
      filtered = filtered.filter((p) =>
        brandIds.includes(String(p.brandId || p.brand_id)),
      );
    }
    if (activeType !== "all") {
      filtered = filtered.filter((p) => {
    ...
    if (activeSeries !== "all") {
      filtered = filtered.filter(
        (p) => p.series === activeSeries || p.category === activeSeries,
      );
    }

    if (categoryFilter) {
      filtered = filtered.filter(
        (p) => p.category === categoryFilter || p.series === categoryFilter,
      );
    }
    ...
    if (strengthFilter) {
      filtered = filtered.filter(
        (p) =>
          p.strength &&
          p.strength.toLowerCase().includes(strengthFilter.toLowerCase()),
      );
    }
    ...
    if (wrapperFilter) {
      ...
      filtered = filtered.filter((p) => {
        const searchString =
          `${p.wrapper || ""} ${p.description || ""}`.toLowerCase();
    ...
    if (flavorFilter) {
      filtered = filtered.filter((p) => {
        const searchString =
          `${Array.isArray(p.flavor_profile) ? p.flavor_profile.join(" ") : ""} ${p.description || ""}`.toLowerCase();
    ...
    if (vibeFilter) {
      filtered = filtered.filter((p) => {
        ...
        if (vibeFilter === "morning")
          return (
            desc.includes("mild") ||
            desc.includes("coffee") ||
            desc.includes("morning")
          );
        if (vibeFilter === "after-dinner")
          return (
            desc.includes("full") ||
            desc.includes("strong") ||
            desc.includes("dinner")
          );
        if (vibeFilter === "celebration")
          return (
            p.price > 500 ||
    ...
    if (inStockOnly) {
      filtered = filtered.filter((p) =>
        p.models?.some((m) => parseInt(m.stock || 0) > 0),
      );
    }
      ...
      filtered = filtered.filter((p) => {
        const isDiscounted =
          ...
          p.models.some(
            (m) =>
              m.original_price &&
              parseFloat(m.original_price) > parseFloat(m.price),
          );
        const isPromoted = activePromos.some((promo) => {
          if (promo.target_type === "all") return true;
          if (
            promo.target_type === "product" &&
            promo.target_id?.split(",").includes(p.id)
          )
            return true;
          if (
            promo.target_type === "brand" &&
            promo.target_id?.split(",").includes(p.brand_id || p.brandId)
          )
            return true;
          return false;
        });
  ...
  const similarProducts = useMemo(() => {
    ...
    let baseList = visibleProducts.filter(
      (p) => !inStockOnly || p.models?.some((m) => parseInt(m.stock || 0) > 0),
    );

    let scored = baseList.map((p) => {
      let score = 0;
      // 1. Same Brand gets points
      if (activeBrand !== "all" && (p.brandId || p.brand_id) === activeBrand)
        score += 5;

      // 2. Strength Proximity
      if (strengthFilter && p.strength) {
        const targetIdx = STRENGTH_ARRAY.findIndex(
          (s) => s.toLowerCase() === strengthFilter.toLowerCase(),
        );
        const pIdx = STRENGTH_ARRAY.findIndex((s) =>
          p.strength.toLowerCase().includes(s.toLowerCase()),
        );
      ...
      if (wrapperFilter && p.wrapper) {
        const targetIdx = WRAPPER_ARRAY.findIndex((w) =>
          w.toLowerCase().includes(wrapperFilter.toLowerCase()),
        );
        const pIdx = WRAPPER_ARRAY.findIndex((w) =>
          p.wrapper.toLowerCase().includes(w.toLowerCase()),
        );
        if (targetIdx !== -1 && pIdx !== -1) {
          const diff = Math.abs(targetIdx - pIdx);
          if (diff === 0) score += 8;
      ...
      if (flavorFilter) {
        ...
        for (const [cluster, flavors] of Object.entries(FLAVOR_CLUSTERS)) {
          if (
            flavors.some((f) =>
              flavorFilter.toLowerCase().includes(f.toLowerCase()),
            )
          ) {
        ...
        if (targetCluster) {
          const pFlavors = Array.isArray(p.flavor_profile)
            ? p.flavor_profile.join(" ")
            : p.description || "";
          if (
            FLAVOR_CLUSTERS[targetCluster].some((f) =>
              pFlavors.toLowerCase().includes(f.toLowerCase()),
            )
          ) {
            score += 8;
          }
      ...
      if (score === 0) {
        ...
        if (
          searchQuery &&
          p.name.toLowerCase().includes(searchQuery.split(" ")[0].toLowerCase())
        )
          score += 3;
      }
  ...
  const availableBrands = useMemo(() => {
    ...
    const activeBrandIds = new Set(
      typeProducts.map((p) => p.brandId || p.brand_id),
    );
    ...
    return allBrands
      ...
      .map((b) => {
        ...
        if (!b.logo) {
          const firstProd = typeProducts.find(
            (p) => (p.brandId || p.brand_id) === b.id && p.image,
          );
  ...
  const getSeriesForBrand = (brandId) => {
    ...
    return Array.from(allSeriesMap[brandId])
      .map((sId) => {
        const sample = products.find((p) => p.series === sId);
        const name = sample
          ? sample.name.replace("Oliva ", "").replace("Melanio", "Melanio")
          : sId;
    ...
    <>
      ...
      {/* Brand Carousel - Show brands, filter to active brand if one is selected */}
      {
        <div className={styles.brandCarouselSection}>
          ...
          <div
            className={styles.brandCarouselWrapper}
            style={{ position: "relative" }}
          >
            <div className={styles.brandCarousel} ref={carouselRef}>
              {availableBrands
                .filter((b) => activeBrand === "all" || b.id === activeBrand)
                .map((brand) => (
                  <div
                    key={brand.id}
      ...
      <div className={styles.layout}>
        ...
        <div
          className={`${styles.sidebarWrapper} ${showFilters ? styles.showFilters : ""}`}
        >
        ...
        >
            ...
            {filteredProducts.length === 0 && !showingSimilar ? (
              <div className={styles.emptyState}>
                ...
                >
                  ...
                  <button
                    onClick={() => setShowingSimilar(true)}
                    className="btn"
                    style={{
          ...
          {Math.ceil(filteredProducts.length / ITEMS_PER_PAGE) > 1 && (
            ...
            >
              ...
              >
                {t("showing")} {(currentPage - 1) * ITEMS_PER_PAGE + 1} &mdash;{" "}
                {Math.min(
                  currentPage * ITEMS_PER_PAGE,
                  filteredProducts.length,
                )}{" "}
                {t("of")} {filteredProducts.length} {t("vitolas")}
              </p>
              <div
                style={{
                  display: "flex",
              ...
              >
                ...
                {[
                  ...Array(Math.ceil(filteredProducts.length / ITEMS_PER_PAGE)),
                ].map((_, i) => {
                  const pageNum = i + 1;
                  ...
                  if (
                    ...
                    return (
                      <button
                        ...
                        style={{
                          ...
                          fontFamily: "var(--font-serif)",
                          fontSize:
                            currentPage === pageNum ? "1.4rem" : "1.1rem",
                          color:
                            currentPage === pageNum
                              ? "var(--color-accent)"
                              : "#fff",
                    ...
                    return (
                      <span key={pageNum} style={{ color: "#666" }}>
                        &hellip;
                      </span>
                    );
                  }
...
function ShopProductCard({ product, t, activePromos = [] }) {
  ...
  const brandName = brands.find(
    (b) => b.id === (product.brandId || product.brand_id),
  )?.name;
  const totalStock = product.models
    ? product.models.reduce((acc, m) => {
        const stockStr =
          m.stock === undefined || m.stock === null || m.stock === ""
            ? "0"
            : m.stock;
  ...
  const hasPromo = activePromos.some((promo) => {
    ...
    if (
      promo.target_type === "product" &&
      promo.target_id?.split(",").includes(product.id)
    )
      return true;
    if (
      promo.target_type === "brand" &&
      promo.target_id?.split(",").includes(product.brand_id || product.brandId)
    )
      return true;
    return false;
  });
    ...
    >
      ...
      >
        <div className={styles.imageContainer}>
          {isOut && (
            <div
              style={{
                ...
                left: 10,
                background:
                  "linear-gradient(135deg, rgba(208, 200, 185, 0.9), rgba(197, 163, 92, 0.9))",
                color: "#120C0A",
                padding: "4px 10px",
                fontSize: "0.75rem",
                fontWeight: "bold",
          ...
          <div
            style={{
              ...
              left: "10px",
              zIndex: 10,
              display: isOut || isLowStock ? "none" : "block",
            }}
          >
          ...
          {product.rating && (
            ...
            >
              {String(product.rating).match(/^\d+/)
                ? String(product.rating).match(/^\d+/)[0]
                : "90"}
            </div>
          )}
          ...
          {(() => {
            const finalBadges = (product.badges || []).filter(
              (b) => typeof b !== "string" || !/%\s*off/i.test(b),
            );
            if (hasDiscount && discountPercent > 0)
              finalBadges.push(`${discountPercent}% OFF`);
              ...
              return (
                <div
                  style={{
                    position: "absolute",
                    top: "10px",
                    right: product.rating ? "60px" : "10px",
                    display: "flex",
                    flexDirection: "column",
                    gap: "8px",
                    zIndex: 15,
                ...
                >
                  {finalBadges.map((badge, idx) => {
                    const isDiscount =
                      typeof badge === "string" &&
                      (/%\s*off/i.test(badge) || badge === "PROMO");
                    return (
                      <span
                        key={idx}
                        className={isDiscount ? "badge-red" : "badge-wind"}
          ...
          <div className={styles.overlay}>
            <div className={styles.overlayContent}>
              <span className={`${styles.quickViewBtn} tour-view-details-btn`}>
                {isOut ? t("notify_me") || "Waitlist" : t("view_details")}
              </span>
            </div>
          </div>
          <div
        ...
        <div className={styles.cardContent}>
          ...
          <div
            className={`${styles.cardHoverInfo} ${isHoverExpanded ? styles.expandedInfo : ""}`}
          >
              ...
              {hasDiscount ? (
                <>
                  ...
                  <p
                    ...
                    style={{ color: "#ff4d4d", marginTop: "2px" }}
                  >
                    {t("from")} EGP {startPrice.toLocaleString()}{" "}
                    <span
                      style={{
                        fontSize: "0.75rem",
                        background: "#ff4d4d",
