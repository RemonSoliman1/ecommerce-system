const fs = require('fs');
let content = fs.readFileSync('components/ui/ProductCard.js', 'utf8');
...
if (!content.includes(exactBottomMatch)) {
    console.log("Could not find bottom match!");
    // Dump the last 25 lines so I can see what it actually is
    const lines = content.split('\n');
    console.log(lines.slice(-25).join('\n'));
} else {
    content = content.replace(exactBottomMatch, replacementBottom);
    
    // Also fix the cardFooter flex layout
